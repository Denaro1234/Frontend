...
export class EditCourseComponent implements OnInit {
  ...
  fetchCourseData(id: number): void {
    this.http.get<Course>(`http://localhost:3000/api/v1/courses/${id}`).subscribe({
      next: (data: Course) => {
        this.course = data;
      },
  ...
  onSubmit(): void {
    if (this.courseId) {
      this.http.put(`http://localhost:3000/api/v1/courses/${this.courseId}`, this.course).subscribe({
        next: () => {
  ...
  deleteCourse(): void {
    if (confirm('Are you sure you want to delete this course?')) {
      if (this.courseId) {
        ...
        this.http.patch(`http://localhost:3000/api/v1/courses/${this.courseId}`, { Class_Status: '0' }).subscribe({
          next: () => {
