...
export class CallSheetComponent implements OnInit {
  ...
  private apiUrl = 'http://localhost:3000/api/v1/s_pre_req';
  ...
  updateRow(row: CallSheetRow): void {
    ...
    if (row.EOD === 'Move to dump') {
      this.http.post<CallSheetRow>('http://localhost:3000/api/v1/dump', rowToUpdate).subscribe({
        next: () => {
    ...
    if (row.EOD === 'Move to call later') {
      this.http.post<CallSheetRow>('http://localhost:3000/api/v1/call-later', rowToUpdate).subscribe({
        next: () => {
          this.http.delete(`${this.apiUrl}/${row.Recruitment_Data_ID}`).subscribe({
            next: () => {
