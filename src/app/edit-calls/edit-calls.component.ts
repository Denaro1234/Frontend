...
export class EditCallsComponent implements OnInit {
  ...
  fetchRow(id: string): void {
    this.http.get<CallSheetRow>(`https://backenddeployment-production-a4eb.up.railway.api/api/v1/s_pre_req/${id}`).subscribe({
  ...
  updateStudent(): void {
    this.http.put(`https://backenddeployment-production-a4eb.up.railway.api/api/v1/s_pre_req/${this.row.Recruitment_Data_ID}`, this.row).subscribe({
      next: () => {
