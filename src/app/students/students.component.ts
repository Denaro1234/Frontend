...
export class StudentsComponent implements OnInit {
  ...
  fetchData(): void {
    this.http.get<StudentDisplay[]>('http://localhost:3000/api/v1/students/display').subscribe({
  ...
  updateRow(student: StudentDisplay): void {
    ...
    if (student.isNew) {
      this.http.post('http://localhost:3000/api/v1/students', studentToUpdate).subscribe({
        next: () => {
      ...
      this.http.put(`http://localhost:3000/api/v1/students/${student.Index_Number}`, studentToUpdate).subscribe({
        next: () => {
  ...
  deleteRow(id: number | null): void {
    ...
    if (confirm('Are you sure you want to delete this entry?')) {
      this.http.patch(`http://localhost:3000/api/v1/students/${id}`, { Updated_By: 'System' }).subscribe({
        next: () => {
